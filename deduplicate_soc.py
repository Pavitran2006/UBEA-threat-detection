...
for i, line in enumerate(lines):
    if "# ---------------- Advanced SOC Dashboard Endpoints ----------------" in line:
        stop_index = i
        break
...
if stop_index != -1:
    ...
    soc_segment = """# ---------------- Advanced SOC Dashboard Endpoints ----------------
...
"""
    new_lines.append(soc_segment)
    
    with open(file_path, 'w') as f:
        f.writelines(new_lines)
    print("Deduplicated SOC segments successfully.")
else:
    print("Could not find SOC segment start. No changes made.")
